...
import { LexiconItem, INTRO_THEME, HOOK_THEME, AUDIO, VIDEO_CONCEPT_THEME, STATIC_CONCEPT_THEME, CONCEPT_CLUSTER, CONCEPT_THEME_GUIDES } from './lexiconData';
...
// Theme
const VIDEO_THEME_OPTIONS = ['BRAND', 'CEL', 'PERF', 'TSTMN', 'VIRAL'];
const STATIC_THEME_OPTIONS = ['BRAND', 'CEL', 'PERF', 'TSTMN'];
...
function App() {
  ...
  const [lexiconPage, setLexiconPage] = useState<'lexicon' | 'concept-themes'>('lexicon');
  ...
  const CONCEPT_THEME_GUIDES_FILTERED = useMemo(() => {
    if (!lexiconQueryNormalized) return CONCEPT_THEME_GUIDES;
    return CONCEPT_THEME_GUIDES.filter((item) => {
      const hay = [
        item.code,
        item.title,
        item.formats,
        ...
        item.summary,
        item.note ?? '',
        ...item.whatIs.flatMap((step) => [step.label, step.desc]),
        ...item.dos,
        ...item.donts,
      ]
        .join(' ')
        .toLowerCase();
      return hay.includes(lexiconQueryNormalized);
    });
  ...
  // Static Concept # input should be numeric only (digits + optional dot).
  // Users enter ONLY the number (e.g. 1.01, 21.03, 111.01). The generator prefixes statS automatically.
  const sanitizeStaticConceptNumber = (raw: string): string => {
    if (!raw) return '';

    // If someone pastes legacy values like stat101 / statS101.01, strip the prefix first.
    let s = raw.replace(/^stat\s*s?/i, '');
    ...
    if (firstDot !== -1) {
      s = s.slice(0, firstDot) + '.' + s.slice(firstDot + 1).replace(/\./g, '');
    }
  ...
  const handleConceptChange = (v: string) => {
    if (mode === 'static') updateField('assetId', sanitizeStaticConceptNumber(v));
    else updateField('assetId', v);
  };
        ...
        <div className="p-8 border-b border-zinc-900">
          <div className="flex items-center gap-4">
             ...
             <div className="flex flex-col leading-none">
               ...
               <p className="text-[#ffff99] text-[10px] font-bold uppercase tracking-[0.25em]">MKT Team V1.26</p>
             </div>
          </div>
        </div>
                  ...
                  <section>
                    <div className="grid grid-cols-2 gap-6 mb-6">
                      <InputField
                        ...
                        placeholder={mode === 'video' ? 'vid025.01' : '11.01'}
                      />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-in fade-in duration-300">
           <div className="bg-zinc-950 border border-zinc-800 w-full max-w-5xl rounded-2xl relative shadow-[0_0_100px_rgba(255,255,153,0.15)] flex flex-col max-h-[90vh] overflow-hidden">
              ...
              <div className="flex items-center justify-between p-6 border-b border-zinc-800 bg-zinc-900/80">
                ...
                <button onClick={() => { setIsLexiconOpen(false); setLexiconQuery(''); setLexiconPage('lexicon'); }} className="text-zinc-500 hover:text-white transition-colors bg-black/40 p-2 rounded-full hover:bg-zinc-800">
                  <X size={24} />
                </button>
              </div>

              <div className="overflow-y-auto custom-scrollbar bg-black/50">
                <div className="sticky top-0 z-20 bg-zinc-950/95 backdrop-blur-xl border-b border-zinc-900 shadow-[0_18px_40px_rgba(0,0,0,0.45)]">
                  <div className="px-10 pt-6 pb-5 space-y-5">
                    <div className="flex items-end justify-between gap-6">
                      <div>
                        <div className="text-zinc-500 text-xs font-bold uppercase tracking-[0.24em] mb-3">Pages</div>
                        <p className="text-sm text-zinc-400 max-w-2xl leading-relaxed">
                          {lexiconPage === 'lexicon'
                            ? 'Search codes, titles, and naming references without leaving the modal.'
                            : 'Review concept-theme strategy, goal, and structure in a cleaner two-page layout.'}
                        </p>
                      </div>

                      <div className="flex items-center gap-3 bg-black/30 border border-zinc-800 rounded-2xl p-2 shrink-0">
                        <button
                          type="button"
                          onClick={() => setLexiconPage('lexicon')}
                          className={`px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-[0.2em] transition-all ${
                            lexiconPage === 'lexicon'
                              ? 'bg-[#ffff99] text-black shadow-[0_0_24px_rgba(255,255,153,0.18)]'
                        ...
                        <button
                          type="button"
                          onClick={() => setLexiconPage('concept-themes')}
                          className={`px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-[0.2em] transition-all ${
                            lexiconPage === 'concept-themes'
                              ? 'bg-[#ffff99] text-black shadow-[0_0_24px_rgba(255,255,153,0.18)]'
                              : 'text-zinc-400 hover:text-white hover:bg-zinc-900'
                          }`}
                        >
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-zinc-500 text-xs font-bold uppercase tracking-[0.2em] shrink-0">Search</div>
                      <input
                        value={lexiconQuery}
                        onChange={(e) => setLexiconQuery(e.target.value)}
                        placeholder={lexiconPage === 'lexicon' ? 'Type to filter (code, title, description)...' : 'Filter themes, steps, dos and donts...'}
                  ...
                  {lexiconPage === 'lexicon' ? (
                    ...
                    <div className="space-y-8">
                      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                        {CONCEPT_THEME_GUIDES_FILTERED.map((theme) => (
                          <article
                            key={theme.code}
                            className="rounded-[28px] border border-zinc-800 bg-black/70 p-8 shadow-[0_0_0_1px_rgba(255,255,255,0.015)]"
                          >
                            <div className="flex items-start justify-between gap-4 mb-6">
                              <div>
                                <div className="text-[#ffff99] font-mono text-3xl font-bold tracking-tight mb-2">{theme.code}</div>
                                <h4 className="text-white text-2xl font-bold leading-tight">{theme.title}</h4>
                              </div>
                              <span className="shrink-0 text-[11px] font-bold uppercase tracking-[0.24em] text-zinc-400 border border-zinc-800 rounded-full px-4 py-2">
                                {theme.formats}
                              </span>
                            </div>

                            <div className="mb-6">
                              <div className="text-zinc-500 text-[11px] font-bold uppercase tracking-[0.22em] mb-2">Goal</div>
                              <p className="text-zinc-200 text-lg leading-relaxed">{theme.goal}</p>
                              <p className="text-zinc-500 text-sm leading-relaxed mt-3">{theme.summary}</p>
                            </div>

                            {theme.whatIs.length > 0 ? (
                              <div className="mb-6">
                                <div className="text-zinc-500 text-[11px] font-bold uppercase tracking-[0.22em] mb-3">What Is</div>
                                <div className="space-y-3">
                                  {theme.whatIs.map((step) => (
                                    <div
                                      key={`${theme.code}-${step.label}`}
                                      className="grid grid-cols-[88px,1fr] gap-4 rounded-2xl border border-zinc-800 bg-zinc-950/70 px-4 py-4"
                                    >
                                      <div className="text-[#ffff99] text-xs font-bold uppercase tracking-[0.22em]">{step.label}</div>
                            ...
                            {theme.note ? (
                              <div className="mb-6 rounded-2xl border border-[#ffff99]/20 bg-[#ffff99]/5 px-4 py-4 text-sm leading-relaxed text-zinc-300">
                                {theme.note}
                              </div>
                            ) : null}

                            {(theme.dos.length > 0 || theme.donts.length > 0) ? (
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-5">
                                  <div className="text-[#ffff99] text-[11px] font-bold uppercase tracking-[0.22em] mb-3">Do</div>
                                  <ul className="space-y-2 text-sm text-zinc-300 leading-relaxed">
                                    {theme.dos.map((item) => (
                                      <li key={`${theme.code}-do-${item}`} className="flex items-start gap-2">
                                        <ChevronRight size={14} className="mt-1 shrink-0 text-[#ffff99]" />
                                        <span>{item}</span>
                                      </li>
                                    ))}
                                  </ul>
                                ...
                                <div className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-5">
                                  <div className="text-zinc-400 text-[11px] font-bold uppercase tracking-[0.22em] mb-3">Dont</div>
                                  <ul className="space-y-2 text-sm text-zinc-400 leading-relaxed">
                                    {theme.donts.map((item) => (
                                      <li key={`${theme.code}-dont-${item}`} className="flex items-start gap-2">
                                        <ChevronRight size={14} className="mt-1 shrink-0 text-zinc-500" />
                                        <span>{item}</span>
                                      </li>
                                    ))}
                                  </ul>
                      ...
                      {CONCEPT_THEME_GUIDES_FILTERED.length === 0 ? (
                        <div className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-8 text-zinc-500 text-sm">
                          No concept themes matched that filter. Tiny gremlin, zero results.
                        </div>
                      ) : null}
                    </div>
                  )}
